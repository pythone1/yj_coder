...
if __name__ == '__main__':
    # datapath = r'F:\20240603\0703\宜兴'
    # os.chdir(datapath)
    # polygon_file = glob.glob('*宜兴*.shp')[0]
    # # polygon_file = r'D:\项目数据\江苏省\疑点核查\江苏省_无锡市_宜兴市_out.shp'
    ...
    # # zdsm_file = r'C:\0924Sentinel处理\射阳进度统计用\总-宜兴.txt'
    # # zdsm = readZDSM(zdsm_file)
    ...
    # 宜兴未填报落在太湖3km的数量
    file1 = r'F:\20240603\0716\宜兴\疑点分析结果2.json'
    file2 = r'F:\20240603\region\宜兴\退养区\退养区.shp'
    # file2 = r'F:\20240603\region\宜兴\无锡市1-禁养区.shp'
    chitang = gpd.read_file(file1)
    chitang = chitang[chitang['status'] == '已填报养殖'].reset_index()
    # chitang.drop('index_right')
    tuiyang1 = gpd.read_file(file2).to_crs(chitang.crs)
    idx = gpd.sjoin(chitang,tuiyang1).index.values
    flg = np.array([False] * len(chitang))
    flg[idx] = True
    chitang1 = chitang[flg]
    # chitang1.to_file(file1.replace('.json',f'_未填报且在{os.path.basename(file2)[0:-4]}内.json'),encoding='utf-8',driver='GeoJSON')
    print(len(chitang1))
    print((chitang1['area'].sum())/666.666)