...
def main() -> None:
    ...
    config = ExperimentConfig(
        ...
        am_chain_count=4,
        am_samples_per_chain=1600,
        am_warmup=400,
        am_adapt_start=400,
        am_initial_covariance=0.0013,
        posterior_validation_samples=32,
        parallel_workers=8,
    ...
    if truth_check["mean_nse"] < 0.999999 or truth_check["sse"] > 1e-8:
        raise RuntimeError(
            f"Truth replay check failed before large run: mean_nse={truth_check['mean_nse']}, sse={truth_check['sse']}"
        )

    log_progress("Starting large run (~overnight / long run depending on machine and SWMM runtime)")
    ...
    log_progress("0401 large run ready")
