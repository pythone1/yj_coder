...
def createShpfile_from_geotiff(shpfile, tiffile):
    ...
    # 添加属性列
    newField = ogr.FieldDefn('waterType', ogr.OFTInteger)
...
def getDBWI(b,g):
    ...
    原理：黑臭水体在蓝绿波段上升缓慢，其他水体在该波段上升较较快，通过计算蓝绿波段的差值，可区分黑臭水体。
    b:  反射率蓝光波段
    g:  反射率绿光波段
    '''
    dbwi = (g-b)
...
if __name__ == '__main__':
    # print(1)
    # tiffile = r'D:\Users\Desktop\water_tif\拼接1_transparent_mosaic_group1.tif'
    # geotiffinfo = geotiffread(tiffile)
    # dataarray = geotiffinfo.dataarray
    # print(dataarray.shape)
    # rows, cols, dim = dataarray.shape
    # data_new = np.zeros((rows, cols, dim))
    # data_new[:, :, 0] = dataarray[:, :, 0]
    # data_new[:, :, 1] = dataarray[:, :, 1]
    # data_new[:, :, 2] = dataarray[:, :, 2]
    # geotiffwrite(r'D:\Users\Desktop\water_tif\1.tif', data_new,
    #              geotiffinfo.geo_transform, geotiffinfo.projection)
    # tiffile=r'U:\盐城无人机\f46凌庄河\f46凌庄河_mosaic_group1.tif'
    # geotiffinfo = geotiffread(tiffile)
    # dataarray=geotiffinfo.dataarray.astype('int8')
    # cv2.imwrite(r'U:\盐城无人机\f46凌庄河\2.tif',dataarray)
    # geotiffwrite(r'U:\盐城无人机\f46凌庄河\1.tif',geotiffinfo.dataarray,geotiffinfo.geo_transform,geotiffinfo.projection)
    # shpfile=r'H:\xiangmu\lyg\10.23\ym.shp'
    # outfile=r'H:\xiangmu\lyg\10.23\cj.tif'
    # imgclip_with_shp(tiffile, shpfile, outfile)
    # #
    # import  glob
    # shpfile=r'D:\Desktop\test\A\fanwei.shp'
    # tiffile=r'E:\PY\GEE\YC_REF\RGB\20220309T023549_20220309T024726_T51STS.tif'
    # outfile = r'D:\Desktop\test\A\kt2.tif'
    # imgclip_with_shp(tiffile, shpfile, outfile)
    # createShpfile_from_geotiff(shpfile,tiffile)


    # # 栅格镶嵌
    # tifpath=r'D:\Users\Administrator\Desktop\best_model\tif\test\water_tif'
    # outfile=r'D:\Users\Administrator\Desktop\best_model\tif\test\water_tif\1.tif'
    # rasterMosaic(tifpath, outfile)

    # tifpath = r'I:\sl\南京地区天地图优化\lianyungang_kengtang\1'
    # outfile = r'I:\sl\南京地区天地图优化\lianyungang_kengtang\xiangqian.tif'
    # rasterMosaic(tifpath,outfile)
    # print('镶嵌已完成')



    # tif = r'D:\Users\Administrator\Documents\WXWork\1688858186325806\Cache\File\2022-10\S2A_MSIL2A_20221013T024651_N0400_R132_T50SQD_20221013T061151_REF10m.tif'
    # tifdata = imgpro.geotiffread(tif)
    #
    # b = tifdata.dataarray[2]
    # g = tifdata.dataarray[1]
    # r = tifdata.dataarray[0]
    # dbwi = (g - b)
    #
    # geo_transform = tifdata.geo_transform
    # projection = tifdata.projection
    # tiffile = r'H:\xiangmu\lyg\10.13\11111.tif'
    # # cv2.imwrite(tiffile, dbwi)
    # cv2.imwrite(tiffile, b)
    # # imgpro.geotiffwrite(tiffile, dbwi, geo_transform, projection)




    reffile = r'I:\pyMethod\segment-anything\SAM_paddle_landclassify\s2_moel\nanjingclss_2022-04-18.tif'
    RGBfile = r'I:\pyMethod\segment-anything\SAM_paddle_landclassify\s2_moel\nanjingclss_2022-04-18_rgb.tif'
    stretch_mode = "2%"
    ...
    # tifpath = r'D:\Users\Administrator\Desktop\best_model\tif'
    # outpath= r'D:\Users\Administrator\Desktop\best_model\tif\新建文件夹'
    # pixelnum=512
