...
from config_0401 import CANDIDATE_NODES, MONITOR_NODES, OUTFALL_NODE, STEP_SECONDS, runtime_model_path
...
def _inject_timeseries_into_inp(runtime_inp: str, injection_series: dict[str, np.ndarray], total_process: pd.DataFrame) -> None:
    ...
    lines = inp_path.read_text(encoding="gbk", errors="ignore").splitlines()
...
def worker_evaluate(task: tuple[int, np.ndarray]) -> dict[str, Any]:
    ...
    runtime_inp = runtime_model_path(os.getpid())
