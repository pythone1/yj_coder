...
OUTPUT = BASE / "Jinshiyuan_AI_Production_Blueprint_套模板_最终版_v19.pptx"
...
for idx, slide in enumerate(source.slides):
    ...
    for shape in list(slide.shapes):
        tune_text(shape, idx)
        if idx == 6 and text_of(shape).startswith("工艺介绍：解决厂内物流任务") and shape.top < 1200000:
            shape.top = Emu(1260000)
        if (
