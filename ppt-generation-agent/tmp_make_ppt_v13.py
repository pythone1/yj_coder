...
OUTPUT = BASE / "Jinshiyuan_AI_Production_Blueprint_套模板_可读性修正_v13.pptx"
        ...
        for para in shape.text_frame.paragraphs:
            for run in para.runs:
                ...
                if not text:
                    continue
                if slide_idx >= 1:
                    # Improve readability on dark image backgrounds. Keep the template header added later untouched.
                    run.font.color.rgb = RGBColor(245, 250, 255)
                    if run.font.size:
                        floor = 13 if slide_idx == 1 else 12
                        run.font.size = Pt(max(run.font.size.pt, floor))
                    else:
                        run.font.size = Pt(12)
...
def add_frame(slide, logo_blob, title, scale, width, slide_idx):
    ...
    # Slightly tighter than the previous version to reduce blank header space.
    header_base = 1380000 if slide_idx == 9 else 840000
    header_h = Emu(int(header_base * scale))
    header = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, 0, width, header_h)
    header.fill.solid()
    ...
    footer = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, s(6550000), width, s(300000))
    footer.fill.solid()
