...
def quary_on_local_knowdge(session_id,user_input):
    ''' 
    本地知识问答 
    ...
    for i,doc in enumerate(documents):
        ...
        print(f"\n\n正在检索第{i+1}个文档：{dbname}...")

        # 加载历史对话
        agt.load_session_history(doc.page_content)

        # 问答
        assistant_message = agt.qa(user_input)
    ...
    assistant_message = agt.client.chat.completions.create(
        model = agt.model,
...
def load_context_from_db(db_faiss_path,kw=''):    
    ''' 加载本地知识库 '''    
    embeddings = HuggingFaceEmbeddings(model_name = 'sentence-transformers/all-mpnet-base-v2')
    db = FAISS.load_local(db_faiss_path,embeddings,allow_dangerous_deserialization=True)
    all_docs = db.similarity_search(kw, k=db.index.ntotal)
...
app = Flask(__name__)
@app.route('/api/aiqa',methods=['GET'])
def crab_farming_technology_api():
    try:
        # read parameters
        ...
        # QA        
        assistant_message = quary_on_local_knowdge(session_id,user_input)
        response = json.dumps({'agent':assistant_message,'code':200}, ensure_ascii=False)
        return response
