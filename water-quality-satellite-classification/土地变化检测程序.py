...
class MainDialog(QDialog):
    ...
    def openImgPath(self):
        ...
        存储用户打开的文件路径，
        添加标签到文件显示清单"""
        tiffile_path = QFileDialog.getExistingDirectory(self, '选择文件夹', r'D:\Users\Administrator\Desktop\best_model\tif')
        # 取出三种后缀的图像
    ...
    def selectModelFile(self):
        """"用于存储用户选择模型文件路径"""
        model_path = QFileDialog.getExistingDirectory(self, '选择文件夹', r'D:\Users\Administrator\Desktop\Water_Classify\inference_model_1')
    ...
    def water_classify_pro(self):
        ...
        hearder2.hide()
        modelfile = self.ui.modelfile
        pixelnum = self.ui.pixelnum

        model = pdx.load_model(modelfile)
        print("模型加载完成")
        for i,key in enumerate(self.ui.file_dic.keys()):
            ...
            outpath_tif = os.path.join(dirpath, "water_tif")
            if not os.path.exists(outpath_tif):
                os.mkdir(outpath_tif)
            self.ui.outpath_tif=outpath_tif
            outpath_shp = os.path.join(dirpath, "water_shp")
            if not os.path.exists(outpath_shp):
                os.mkdir(outpath_shp)
            out_tif = os.path.join(outpath_tif, basename + "水域" + ".tif")
            out_shp = os.path.join(outpath_shp, basename + "水域" + ".shp")

            if self.ui.img_tpye == 1:
                geotifinfo = geoClassify(tiffile,pixelnum, model,out_tif,out_shp,outpath_visual,img_tpye=1)
                self.ui.geoinfo_list.append(geotifinfo)
            else:
                geoClassify(tiffile, pixelnum, model, out_tif,out_shp,outpath_visual,img_tpye=2)
...
def geoClassify(tiffile, pixelnum,model,out_tif,out_shp,outpath_visual,img_tpye):
    """
    ...
    # 分块进行模型预测
    result = Main_Classify(dataarray,pixelnum,model,outpath_visual)  # np.int
...
def Main_Classify(im,pixelnum, model,outpath_visual):
    """
    对图像进行分块预测
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # cv2.COLOR_BGR2RGB，读取时为RGB格式，保存时设置保存成RGB格式
        # A_img = cv2.cvtColor(cv2.imread(A_path), cv2.COLOR_BGR2RGB)
        A_img = imgpro.geotiffread(A_path)
        A_img = preProcess(A_img)

        B_img = imgpro.geotiffread(B_path)
        B_img = preProcess(B_img)
        ...
        # print(image.shape)
        # cv2.IMREAD_GRAYSCALE该参数用于灰度图读取灰度图
