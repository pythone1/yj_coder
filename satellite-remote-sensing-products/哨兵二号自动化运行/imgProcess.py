...
datatype：数据存储类型
'''
def imgslice_by_pixels(tifdata,pixelnum,outpath,prefix = "subset_",suffix = ".png",datatype = "UINT8"):
    #横轴完整分块数量
    ...
    for i in range(ynum):       #横轴
        ...
        for j in range(xnum):   #纵轴            
            ...
            if np.nanmax(np.nanmax(np.nanmax(subset))) == 0 or np.nanmin(np.nanmin(np.nanmin(subset))) == 255:
                ...
                subsetfilename = prefix + str(subset_id) + suffix       #裁剪图像保存格式为png
...
if __name__ == '__main__':
    tifpath = r'I:\Sentinel2_DATA\20210620\S2A_MSIL2A_20210620T024551_N0300_R132_T50SQC_20210620T044836.SAFE\GRANULE\L2A_T50SQC_A031303_20210620T025729\IMG_DATA\R10m'
    os.chdir(tifpath)
    tiffile = glob.glob("*TCI*")[0]
    geotiff = geotiffread(tiffile)
    geotiffwrite(r'L:\Sentinel2_DATA\tmp.tif',geotiff.dataarray,geotiff.geo_transform,geotiff.projection,datatype="UINT8")