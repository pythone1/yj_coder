...
# lac = LAC(mode='lac')
SURNAME_PATH = r"F:\工作\江苏省农业厅渔业资源调查\进度统计表\行政区划\常见姓氏.txt"
XZQH = gpd.read_file(r'F:\工作\python\省厅项目进度统计\JiangSu_XZQH.shp')
REGIONCODE = pd.read_excel(r'F:\工作\江苏省农业厅渔业资源调查\进度统计表\行政区划\行政编码.xlsx')
...
def mergeData(xls_file, polygon_file, dels_file=None):
    ...
    try:
        ctxx = pd.merge(ctxx,ctlk.loc[:,['图斑id','TBID','图斑面积']],on=['图斑id'])
    except:
        ctxx = pd.merge(ctxx,ctlk.loc[:,['TBID','图斑面积']],on=['图斑id'])
    ed_time = datetime.now()
    spd_time = (ed_time - st_time).total_seconds()
...
def addAttr2Plygons(ctlk,ctxx):
    ...
    '''
    try:
        ctlk = pd.merge(ctlk,ctxx.loc[:, ['池塘id', '养殖经营人名称','养殖主体类型','状态','状态值','池塘所有权人名称','疑点信息','地址','图斑id']],on='图斑id',how='left')
    except:
        ctlk = pd.merge(ctlk.drop(columns='图斑id'),ctxx.loc[:, ['池塘id', '养殖经营人名称','养殖主体类型','状态','状态值','池塘所有权人名称','疑点信息','地址','图斑id']],on='图斑id',how='left')
...
def locationAnalysis(sjoins, polygons):
    ...
    # 1个图斑有多个点
    dulplicates = getDulplicates(sjoins.loc[~sjoins['ID'].isnull(),'ID'].values)  # ID来自polygon ID
    for d in dulplicates:
        idx = sjoins['ID'] == d
    ...
    # 图斑内无点
    polygons['位置疑点'] = ''
    registered = sjoins.loc[~sjoins['ID'].isnull(),'ID'].values
...
def TBJDTJ04(polygons,fields,outfile):
    '''
    按字段统计数量/进度
    '''
    ndel = polygons['Ndel']
    ...
    cols = tj_qx.columns
    tj_shi = tj_qx.groupby(level=0)[cols].sum()
    for i in tj_shi.index:
        tj_qx.loc[(i,'a'),cols] = tj_shi.loc[i,cols]
    tj_qx = tj_qx.sort_index(ascending=False)
    tj_qx['市'] = tj_qx.index.get_level_values(0)
    tj_qx['区县'] = tj_qx.index.get_level_values(1).str.replace('a','合计')
    tj_qx = tj_qx.set_index(['市','区县'])
    ...
    # 备注删除数量
    idx = tj_qx['备注'] != 0
    tj_qx.loc[idx,'备注'] = '上报' + tj_qx.loc[idx,'备注'].astype('str') + '个图斑删除，该部分不参与进度统计，后续抽样质控后删除'
    tj_qx.loc[~idx,'备注'] = ''
    idx = tj_shi['备注'] != 0
    tj_shi.loc[idx,'备注'] = '上报' + tj_shi.loc[idx,'备注'].astype('str') + '个图斑删除，该部分不参与进度统计，后续抽样质控后删除'
    tj_shi.loc[~idx,'备注'] = ''

    # 导出统计表
    tj_qx.to_excel(outfile)
    tj_shi.to_excel(outfile.replace('.xlsx','-市.xlsx'))
