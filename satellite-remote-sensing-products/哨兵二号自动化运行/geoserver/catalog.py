...
class Catalog:
    ...
    def create_featurestore(self, name, data, workspace=None, overwrite=False,
        ...
        if isinstance(data,dict):
            ...
            archive = data
        print('file open before')
        print(data)
        message = open(archive, 'rb')
        print('file open later')
    ...
    def _create_coveragestore(self, name, data, workspace=None,
        ...
        params = {"configure": "first", "coverageName": name,'InputTransparentColor':"#000000",'OutputTransparentColor':"#000000"}
    ...
    def modify_coverage_trancolor(self,layer_name,workspace):
        headers = {
            "Content-type":"application/xml",
            "Accept":"application/xml"
        }
        ...
        cs_url = urljoin(
            ...
            "workspaces/{}/coveragestores/{}/coverages/{}".format(
                workspace.name,layer_name,layer_name
            )
        )
        #params = {"coverage":{"name":layer_name,"parameters":{"entry":[{"string":["InputTransparentColor","000000"]}]}}} 
        params = '<coverage><name>{}</name><parameters><entry><string>InputTransparentColor</string><string>#000000</string></entry><entry><string>SUGGESTED_TILE_SIZE</string><string>512,512</string></entry></parameters></coverage>'.format(layer_name)
        r = self.session.put(cs_url, data=params, headers=headers)
        self._cache.clear()
        print(r.status_code)
    ...
    def create_coveragestore_external_shp(self, name, data,workspace=None,
                                              overwrite=False):
        self._create_coveragestore_shp(name, data, workspace=workspace,
                                   overwrite=overwrite, external=True)

    def _create_coveragestore_shp(self, name, data, workspace=None,
                              overwrite=False, external=False):
        if not overwrite:
            try:
                store = self.get_store(name, workspace)
    ...
    def modify_layer(self,layer_name, workspace,style_name):
        ...
        cs_url = urljoin(
            self.service_url,
            "workspaces/{}/layers/{}".format(
                workspace.name,layer_name
            )
        )
        print(cs_url)
        params = '<layer><defaultStyle><name>{}</name></defaultStyle></layer>'.format(style_name)
