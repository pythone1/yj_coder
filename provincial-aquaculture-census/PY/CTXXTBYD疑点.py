...
def areaAnalysis2(gdf):
    ...
    for i in htzmj.index:
        gdf.loc[gdf['合并id'] == i, ['合同面积唯一值个数', '合同总面积', '图斑总面积']] = [htmj_num[i], htzmj[i],
                                                                                           tbzmj[i]]
...
def TBJDTJ02(polygons, xzq, outfile, deletes=None):
    ...
    with pd.ExcelWriter(outfile, engine='openpyxl') as writer:
        ...
        tj2.insert(5, '填报进度', ((tj2['已填报养殖个数'] + tj2['已填报非养殖个数']) / tj2['总图斑个数']).apply(
            lambda x: format(x, ".2%")))
        ...
        for s in xzq['市'].unique():
            ...
            t.insert(6, '填报进度', ((t['已填报养殖个数'] + t['已填报非养殖个数']) / t['总图斑个数']).apply(
                lambda x: format(x, ".2%")))
...
def TBJDTJ03(polygons, xzq, outfile):
    ...
    with pd.ExcelWriter(outfile, engine='openpyxl') as writer:
        ...
        for i, row in xzq.iterrows():
            ...
            # p0 = polygons[polygons.intersects(row.geometry)]
            p0 = polygons[(polygons['NAME'] == row['NAME']) & (polygons['市'] == row['市'])].drop_duplicates(
                subset=['TBID'])
            p = p0[p0['Ndel']]
            tj1['总图斑个数'].append(len(p))
            tj1['总图斑面积(亩)'].append(np.round(p["area"].sum() / 666.666, 2))
...
def TBJDTJ04(polygons, fields, outfile, subtotle=True):
    ...
    if subtotle:
        ...
        idx = tj_shi['备注'] != 0
        tj_shi.loc[idx, '备注'] = '上报' + tj_shi.loc[idx, '备注'].astype('int').astype(
            'str') + '个图斑删除，该部分不参与进度统计，后续抽样质控后删除'
        tj_shi.loc[~idx, '备注'] = ''
...
def TBJDTJ05(polygons, fields, outfile, subtotle=True):
    ...
    if subtotle:
        ...
        if len(idx_name) == 1:
            ...
            idx.append('合计')
            tj_shi.loc[tuple(idx), cols] = tj_shi.sum(axis=0)
        tj_shi.insert(4, '填报进度',
                      ((tj_shi['已填报养殖个数'] + tj_shi['已填报非养殖个数']) / tj_shi['总图斑个数']).apply(
                          lambda x: format(x, ".2%")))
        idx = tj_shi['备注'] != 0
        tj_shi.loc[idx, '备注'] = '上报' + tj_shi.loc[idx, '备注'].astype('int').astype(
            'str') + '个图斑删除，该部分不参与进度统计，后续抽样质控后删除'
    ...
    # 填报进度
    tj_qx.insert(4, '填报进度', ((tj_qx['已填报养殖个数'] + tj_qx['已填报非养殖个数']) / tj_qx['总图斑个数']).apply(
        lambda x: format(x, ".2%")))
