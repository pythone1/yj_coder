...
def main() -> None:
    ...
    save_json(truth_check, SMALL_RESULT_DIR / "0401_truth_replay_check.json")
    ...
    if truth_check["mean_nse"] < 0.999999 or truth_check["sse"] > 1e-8:
        raise RuntimeError(
            f"Truth replay check failed before small run: mean_nse={truth_check['mean_nse']}, sse={truth_check['sse']}"
        )
    ...
    final_eval["sim_delta"].to_csv(SMALL_RESULT_DIR / "0401_最终方案模拟增量.csv", index=False, encoding="utf-8-sig")

    summary = {
        "run_mode": "0401 clean small run",
    ...
    (SMALL_RESULT_DIR / "0401_结果汇总.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    log_progress("0401 small run ready")
