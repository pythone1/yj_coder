...
def main() -> None:
    ...
    log_progress("Rebuilding canonical 0417 v2 data from 32h clean baseline and J20/J48/J11 50% event model")
    build_data_main()
    ...
    config = ExperimentConfig(
        ...
        posterior_validation_samples=128,
        parallel_workers=8,
    ...
    save_json(truth_check, OUTPUT_DIR / "0417_truth_replay_check.json")
    ...
    save_csv(ga_all_df, "0417_GA_all.csv")
    save_csv(ga_history_df, "0417_GA_best_by_generation.csv")
    save_csv(ga_last_df, "0417_GA_last_generation.csv")
    ...
    save_csv(initial_ppd_df, "0417_initial_PPD.csv")

    am_df = run_am(dataset, generated, initial_ppd_df, config)
    save_csv(am_df, "0417_AM_samples.csv")

    ppd_samples_df, posterior_df = extract_ppd(am_df, config)
    save_csv(ppd_samples_df, "0417_PPD_samples.csv")
    save_csv(posterior_df, "0417_posterior_node_weights.csv")

    bands_df, coverage_df = posterior_predictive_validation(dataset, generated, ppd_samples_df, config)
    save_csv(bands_df, "0417_posterior_predictive_bands.csv")
    save_csv(coverage_df, "0417_posterior_predictive_coverage.csv")
    ...
    summary = {
        "run_mode": "0417 paper aligned run",
    ...
    save_json(summary, OUTPUT_DIR / "0417_summary.json")
    log_progress("0416 paper-aligned run ready")
