...
def downloadProc(path):    
    ...
    geojson=os.path.join(path0,'SouthYZRiver.geojson')    # 范围
    # geojson=os.path.join(path0,'LYG.geojson')
    # geojson=os.path.join(path0,'BEIJING.geojson')
    ...
    if sentinel2Dow(api,geojson,dates,platformname,processinglevel,cloud) == True:
        ...
        for s2 in s2_files: 
            ...
            # 创建RGB图像金字塔
            cmd_str = r'gdaladdo -ro ' +  RGB_file + ' 1 2 4 6 8 10'
            os.system(cmd_str)
            # 创建有效范围的掩膜              
...
def algoProc1(path):
    ...
    for ref_file in ref_files:             
        ...
        imgpro.geotiffwrite(FUI_file,FUI,waterref.geo_transform,waterref.projection,datatype="FLOAT32")
        imgpro.geotiffwrite(SD_file,SD,waterref.geo_transform,waterref.projection,datatype="FLOAT32") 
        # 创建SD图像金字塔
        cmd_str = r'gdaladdo -ro ' +  SD_file + ' 1 2 4 6 8 10'
        os.system(cmd_str) 
