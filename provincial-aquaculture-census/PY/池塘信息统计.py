...
def readCTXX(ctxx_pth):
    ...
    for f in files:
        print(f)
        df_list.append(pd.read_csv(f,skiprows=1,dtype='str',sep=',',usecols=range(54),index_col=False))
...
def normalizeDF(df):
    ...
    # 养殖品种、产量(顺便删除重名不重要字段)
    df['图斑面积'] = pd.to_numeric(df['图斑面积'],errors='coerce')*0.0015
    ...
    # 有重名品种的新建字段作为汇总
    for p in npz:
        # ini_fieds = [f"{p}产量(吨)",f"淡水{p}产量(吨)",f"海水{p}产量(吨)"]
        # ini_fieds = [f"{p}产量(吨)",f"淡水{p}产量(吨)",f"海水{p}产量(吨)"]
...
if __name__ == '__main__':
    DPTH = r'E:\全省养殖池溏上图入库普查\养殖信息统计\20250527\射阳'

    '''
    数据整理汇总
    '''
    # 池塘信息表 
    zfile = f'{DPTH}\\池塘信息表.zip'
    groupfields = ['养殖经营人名称', '养殖经营人证件号码','地址','养殖主体类型']
    # yzpz = ['鳊鲂','鲫鱼','鲈鱼','泥鳅', '黄鳝', '乌鳢','蛙','乌鳗']
    yzpz = ['鳊鲂','鲫鱼','鲈鱼','泥鳅']
    ...
    #     '黄颡鱼', '黄鳝', '鳜鱼', '所有鲈鱼',
    #     '鲟鱼', '鳗鲡', '罗氏沼虾', '青虾', '克氏原螯虾', '所有南美白对虾', '河蟹',
    #     '河蚌', '龟', '鳖', '蛙', '所有其他种类', '观赏鱼'
    # ]
    # yzpz = None
    # 当日数据路径
    ...
    if os.path.exists(ctxx_file):
        ctxx = pd.read_csv(ctxx_file,engine='python',encoding='utf-8',on_bad_lines='warn')
        # 池塘信息表格式处理
    ...
    # 结果导出
    outfile = f"{outpath}\\上图入库填报信息统计{datetime.now().strftime('%Y%m%d-%H%M%S')}.xlsx"
