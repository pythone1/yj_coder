from imgProcess import imgclip_with_shp
...
def db_insert_map_record(db, satellite_name,format,catalog,sensor,resolution,map_name,cover_name,center_long,center_lat,pro_time):
    try:
        query_sql = "select map_name from data_maps where map_name ='%s'"
        if db.cursor.execute(query_sql % map_name):
            update_sql = "update data_maps set pro_time='%s' where map_name ='%s'" 
            data = (pro_time,map_name)
            db.cursor.execute(update_sql % data)
            db.conn.commit()
            return 0
        else:
            # %s的个数要和参数一样多
...
if __name__=="__main__":
    ...
    def query():
        with MySQLPool() as db:
            map_name = "S2Server:S2A_MSIL2A_20210203T025931_N0214_R032_T50RLT_20210203T055016_SD"
            id ='2718'
            query_sql = "select map_name from data_maps where map_name ='%s'"
            try:
                result = db.cursor.execute(query_sql % map_name)
                print("result1:",db.cursor.fetchone())
            except Exception as e:
                print(str(e))
                print("000000")
    query()
