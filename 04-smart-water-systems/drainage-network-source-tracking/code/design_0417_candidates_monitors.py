...
模块说明: design_0417_candidates_monitors.py - 核心业务算法实现
...
INP_FILES = sorted(MODEL_DIR.glob("*.inp"))
EVENT_INP = next((p for p in INP_FILES if any(k in p.name.lower() for k in ("event", "injection", "rain", "wet", "雨天", "注水", "入流"))), INP_FILES[min(1, len(INP_FILES) - 1)] if INP_FILES else MODEL_DIR / "0520_event.inp")
CLEAN_INP = next((p for p in INP_FILES if any(k in p.name.lower() for k in ("baseline", "clean", "dry", "旱天", "基线", "no_"))), INP_FILES[0] if INP_FILES else MODEL_DIR / "0520_clean_baseline.inp")

HTML_OUT = MODEL_DIR / "0520_scheme_design_candidates_monitors.html"
REPORT_OUT = MODEL_DIR / "0520_scheme_design_report.md"
SUMMARY_JSON = MODEL_DIR / "0520_scheme_design_summary.json"
CONFIG_SNIPPET = MODEL_DIR / "0520_scheme_config_snippet.py"
ALL_NODE_DELTA_CSV = MODEL_DIR / "0520_scheme_all_node_delta_metrics.csv"

RUNTIME_DIR = ROOT / "runtime_0520_scheme_design"
...
def write_outputs(
    ...
    html = [
        ...
        f"<title>0520候选井与监测点设计</title><style>{css}</style></head><body>",
        "<header><h1>0520 候选井与监测点设计方案</h1><p>基于当前配置的真值注入点、事件模型和旱天基线。</p></header><main>",
        "<section class='cards'>",
