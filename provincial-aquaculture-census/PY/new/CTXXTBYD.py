...
def is_company(str_char):
    ...
    pattern2 = ['镇', '村', '街', '社区', '集体', '居委会', '组','农庄','基地','垂钓中心','委员会','墓区','示范区','渔场','蟹业','科技城','研究所',
                '合作社', '养殖场', '农场', '钓场','生产队','处理中心','发展中心','养殖中心','水务站','繁育场','种植场','园艺中心','推广中心',
                '厂', '公司', '集团', '研究院','局','种牛场','部','种蜂场','闸管所','高级中学','服务中心','繁育场','浦泉农科','西埂莲乡','种植中心',
                '禽业','能源','委员会','生态园','林场', '农科院','电站','育苗场','管理所','病院','蚕种场','良种场','土菜馆','生态休闲园',
                '湾景区','农业园区','医院','牧业','风景区','管理所','促进中心','帕蒂亚庄园','猪场','张湾中学','茶场','生态池',
                '人民政府','指导站','经济开发区','张湾乡','闸坝所','管理处','气象台','邓楼果园','丙辰电子','联亚纺织','观光中心',
                '第二中学','水产良种场','养殖','山茶场','鱼种场','金绿生态','示范区','农业','合作联社','渔场','垂钓中心','优来谷成',
                '盱眙国联','淮安市淮安区园艺场','村委会','果园','槐树居','管委会','柳山居','石集乡瓦房居','示范园','国营泗阳林苗圃',
...
def MCAnalysis2(df):
    ...
    for i in df.index:
        ...
        if (nature == '集体/公司') & (not is_company(name)):
...
def locationAnalysis(sjoins, polygons):
    ...
    idx1=sjoins['图斑id']=='/'
    idx2=sjoins['填报状态']=='已填报养殖'
    sjoins.loc[idx1 & idx2, '位置疑点'] = '无对应图斑'
    # 1个图斑有多个点
    dulplicates = getDulplicates(sjoins.loc[sjoins['图斑id']!='/','图斑id'].values)  # ID来自polygon ID
    for d in dulplicates:
        idx = sjoins['图斑id'] == d
        if (sjoins.loc[idx,'填报状态']=='已填报养殖').any():
