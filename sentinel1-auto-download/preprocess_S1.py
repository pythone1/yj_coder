...
def GRDPreProcess(file_list,outpath,subsetregion=None):
    '''
    ...
    if len(product_list) == 1:
        ...
        # 若只有一张影像，输出文件与输入文件同名
        bansename = os.path.basename(file_list[0]).replace('.zip','.tif')
        outfile = os.path.join(outpath,bansename)
    else:
        assembly = sliceMosaic(product_list)
        # 若有多张影像拼接，输出文件取日期关键字
        sensedate = os.path.basename(file_list[0]).split('_')[4].split('T')[0]
        outfile = os.path.join(outpath, '_'.join(['S1', 'GRDH', sensedate, time.strftime('%Y%m%dT%H%M%S')]) + '.tif')
    ...
    # 写为tif
    write2File(outfile,db,format='GeoTIFF')
...
GRDPreProcess([file_list], outpath, subsetregion)
