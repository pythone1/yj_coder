...
def save_solution_outputs(
    ...
    for name, shares in solutions.items():
        ...
        result["sim_delta"].to_csv(output_dir / f"0417_solution_{name}_delta.csv", index=False, encoding="utf-8-sig")
        result["event_monitor"].to_csv(output_dir / f"0417_solution_{name}_event_monitor.csv", index=False, encoding="utf-8-sig")
        result["event_outlet"].to_csv(output_dir / f"0417_solution_{name}_outlet.csv", index=False, encoding="utf-8-sig")
        score_rows.append(
            {
                "solution": name,
    ...
    score_df.to_csv(output_dir / "0417_solution_scores.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(share_rows).to_csv(output_dir / "0417_solution_shares.csv", index=False, encoding="utf-8-sig")
    ...
    recommended_result["sim_delta"].to_csv(output_dir / "0417_final_solution_delta.csv", index=False, encoding="utf-8-sig")
    recommended_result["event_monitor"].to_csv(output_dir / "0417_final_solution_event_monitor.csv", index=False, encoding="utf-8-sig")
    recommended_result["event_outlet"].to_csv(output_dir / "0417_final_solution_outlet.csv", index=False, encoding="utf-8-sig")
