...
- `index.html`：主页面和视图容器。
- `js/resume.js`：在线简历编辑与导出，简历页支持编辑状态栏、评分状态、区块快速跳转、会话内撤销/恢复、持久化版本历史和版本差异对比；Word、HTML、Markdown、JSON 导出统一使用 UTF-8，PDF/打印使用独立简历纸张窗口。
- `js/portfolio.js`：求职项目作品集数据与渲染。
- `js/radar.js`：前沿 AI 工程知识雷达，跟踪新概念、求职相关度、建设动作和来源链接。
- `js/knowledge.js`：AI、遥感、数据工程、前沿 AI 工程知识库，知识卡支持复制整卡、面试话术和代码片段。
- `js/quiz.js`：模拟面试题库。
- `js/app.js`：应用状态、导航、搜索、卡片、PPT 等交互逻辑。
- `css/style.css`：全局样式和作品集页面样式。
- 全局搜索：统一检索知识库、知识雷达、作品集和面试题，支持最近搜索、常用入口、结果面板、上下键选择、Enter 打开和 Esc 关闭。
- 数据中心：浏览器本地持久化状态、关键操作新鲜度提示、全量 JSON 备份/恢复、恢复前差异预览、用户配置、安全重置、简历版本历史、岗位 JD 匹配分析和求职材料报告导出入口。
