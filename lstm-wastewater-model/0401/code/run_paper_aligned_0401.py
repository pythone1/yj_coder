...
OUTPUT_DIR = RESULT_DIR / "paper_aligned_run"
...
def main() -> None:
    ...
    # Paper-aligned GA where the paper provides explicit settings:
    # 5 populations, 80 individuals, 25 generations, competition/migration every 5 generations.
    # The paper does not state elite ratio / mutation strength / migration count precisely,
    # so we keep conservative engineering values here to reduce premature homogenization.
    config = ExperimentConfig(
        ga_population_count=5,
        ga_population_size=80,
        ga_generations=25,
        ga_elite_ratio=0.12,
        ga_mutation_strength=0.20,
        ...
        am_warmup=500,
        am_adapt_start=500,
        am_initial_covariance=0.0012,
        posterior_validation_samples=40,
        parallel_workers=8,
    ...
    if truth_check["mean_nse"] < 0.999999 or truth_check["sse"] > 1e-8:
        raise RuntimeError(
            f"Truth replay check failed before paper-aligned run: mean_nse={truth_check['mean_nse']}, sse={truth_check['sse']}"
        )

    log_progress("Starting paper-aligned run")
    ...
    summary = {
        "run_mode": "0401 paper-aligned run",
        ...
        "paper_alignment_notes": {
            "ga_population_count": "aligned_to_paper",
            "ga_population_size": "aligned_to_paper",
            "ga_generations": "aligned_to_paper",
            "ga_migration_interval": "aligned_to_paper",
            "ga_elite_ratio": "engineering_choice_not_specified_in_paper",
            "ga_mutation_strength": "engineering_choice_not_specified_in_paper",
            "ga_migration_count": "engineering_choice_not_specified_in_paper",
            "ga_competition_replace_count": "engineering_choice_not_specified_in_paper",
            "am_chain_count": "engineering_extension",
        },
    ...
    log_progress("0401 paper-aligned run ready")
