...
from config_clean import CANDIDATE_NODES, MONITOR_NODES, ExperimentConfig
from simulation_clean import ExperimentDataset, simplex_project, worker_evaluate, worker_initializer
...
def run_ga(dataset: ExperimentDataset, generated_data: dict[str, pd.DataFrame], config: ExperimentConfig) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, np.ndarray]:
    ...
    for generation in tqdm(range(config.ga_generations), desc="GA generations", leave=True):
        ...
        for pop_idx, pop in enumerate(populations):
            ...
            log_progress(
                f"GA generation {generation + 1}: population {pop_idx + 1} best_nse={best['mean_nse']:.4f}"
            )
...
def roulette_initial_ppd(merged_last: pd.DataFrame, ga_best_shares: np.ndarray, config: ExperimentConfig, rng: np.random.Generator) -> pd.DataFrame:
    if merged_last.empty:
        raise ValueError("GA last generation is empty")
    ...
    log_progress(f"Initial PPD built: keep_count={len(initial)}")
...
def run_am(dataset: ExperimentDataset, generated_data: dict[str, pd.DataFrame], initial_ppd: pd.DataFrame, config: ExperimentConfig) -> pd.DataFrame:
    ...
    for chain_id in range(config.am_chain_count):
        ...
        accepted_count = 0
        for step in tqdm(
            range(config.am_samples_per_chain),
            desc=f"AM chain {chain_id + 1}",
            leave=True,
        ):
            ...
            rows.append(row)
        log_progress(
            f"AM chain {chain_id + 1} done: accept_rate={accepted_count / max(1, config.am_samples_per_chain):.4f}"
        )
...
def extract_ppd(am_samples: pd.DataFrame, config: ExperimentConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if tail.empty:
        raise ValueError("AM tail samples are empty")
    summary_rows = []
...
def posterior_predictive_validation(
    ...
    for draw_idx, (_, draw) in enumerate(
        tqdm(draws.iterrows(), total=sample_count, desc="Posterior validation", leave=True),
        start=1,
    ):
