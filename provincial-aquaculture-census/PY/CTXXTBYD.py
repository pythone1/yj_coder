...
def addAttr2Plygons(ctlk, ctxx):
    try:
        ctlk = pd.merge(ctlk,ctxx.loc[:, ['养殖方式','池塘id', '养殖经营人名称','养殖主体类型','身份证号','统一社会信用代码','状态','状态值','养殖证编号','池塘所有权','池塘所有权人名称','池塘土地属性','地址','图斑id','手机号码','地址','池塘位置','联系人','联系方式','承包期限','合同面积','净水面面积','养殖品种/预计亩产量','是否完成池塘标准化改造','尾水集中排放期','清塘淤泥处理方式','处置频率','用途','排口位置','有无尾水处理','尾水处理工艺','尾水净化区面积']],on='图斑id',how='left')
    except:
        ctlk = pd.merge(ctlk.drop(columns='图斑id'),ctxx.loc[:, ['养殖方式','池塘id', '养殖经营人名称','养殖主体类型','身份证号','统一社会信用代码','状态','状态值','养殖证编号','池塘所有权','池塘所有权人名称','池塘土地属性','地址','图斑id','手机号码','地址','池塘位置','联系人','联系方式','承包期限','合同面积','净水面面积','养殖品种/预计亩产量','是否完成池塘标准化改造','尾水集中排放期','清塘淤泥处理方式','处置频率','用途','排口位置','有无尾水处理','尾水处理工艺','尾水净化区面积']],on='图斑id',how='left')
...
def is_company(str_char):
    ...
    pattern2 = ['镇', '村', '街', '社区', '集体', '居委会', '组','农庄','基地','垂钓中心','委员会','墓区','示范区','渔场','蟹业','农科所',
                ...
                '人民政府','指导站','经济开发区','张湾乡','闸坝所','管理处','气象台','邓楼果园','丙辰电子','联亚纺织',
                '第二中学','水产良种场','养殖','山茶场','鱼种场','金绿生态','盱眙国联','淮安市淮安区园艺场','村委会',
                '果园','槐树居','管委会','柳山居','石集乡瓦房居','示范园','国营泗阳林苗圃',
                '荷香园','科技示范园','产业园','联鑫钢构','水利站','管理中心','农技中心','铜山区三河尖矿']
    for p in pattern2:
        if p in str_char:
            return True
    return False
...
def MCAnalysis(df):
    for i in df.index:
        nature = str(df.loc[i, '池塘所有权'])
        name = str(df.loc[i, '池塘所有权人名称'])  # 确保 name 不是 NaN
        if (nature != '其他') and ('家庭农场' in name):
            yd = ''
        else:
            # 所有权为个人，所有权人名称不是人名
        ...
        nature = str(df.loc[i, '养殖主体类型'])
        name = str(df.loc[i, '养殖经营人名称'])
        if (nature != '其他') & ('家庭农场' in name):
            ...
            # 联系人不是人名
            name = str(df.loc[i, '联系人'])
            if not is_person(name):
                yd += '联系人疑似填写有误（非人名）,'

            # 人名疑似笔误
            if is_person_similar(str(df.loc[i, '池塘所有权人名称']), str(df.loc[i, '养殖经营人名称'])) and (str(df.loc[i, '池塘所有权人名称']) != str(df.loc[i, '养殖经营人名称'])):
                yd += '人名疑似笔误,'
...
def TBJDTJ04(polygons,fields,outfile,subtotle=True):
    ...
    polygons.loc[ndel,'总图斑'] = polygons.loc[ndel,'area']
    polygons.loc[ndel & ytb,'已填报'] = polygons.loc[ndel & ytb,'area']
    polygons.loc[ndel & cjd,'村已校对'] = polygons.loc[ndel & cjd,'area']
    polygons.loc[ndel & zjd,'镇已校对'] = polygons.loc[ndel & zjd,'area']
    polygons.loc[ndel & xjd,'县已校对'] = polygons.loc[ndel & xjd,'area']
    polygons.loc[sc,'备注'] = polygons.loc[sc,'area']
...
def TBJDTJ05(polygons,fields,outfile,subtotle=True):
    ...
    sc = polygons['Ndel']==False
    polygons.loc[ndel,'总图斑'] = polygons.loc[ndel,'area']
    polygons.loc[ndel & tbyz,'已填报养殖'] = polygons.loc[ndel & tbyz,'area']
    polygons.loc[ndel & tbfyz,'已填报非养殖'] = polygons.loc[ndel & tbfyz,'area']
    polygons.loc[ndel & wtb,'未填报'] = polygons.loc[ndel & wtb,'area']
    polygons.loc[sc,'备注'] = polygons.loc[sc,'area']
...
def generateMapByDistrict(ct,xzq,xlsfile,outpath,name):
    '''
    按行政区生成地图网页和表格
    ct: gpd.GeoDataFrame 池塘图斑
    xzq: gpd.GeoDataFrame 行政区图斑
    ...
    for i,row in xzq.iterrows():
        ...
        # intersects = ct[ct.intersects(geom)]
        intersects_map = createCTMap(ct,geom)
        intersects_map.save(f'{outpath}\\{name}-池塘分布.html')


        # workbook = openpyxl.load_workbook(xlsfile)
        # sheet = workbook.active
        # for j,v in enumerate(ct['TBID'].values):
        #     sheet.cell(6+j,3).value = v.replace(',','')
...
def mclAnalysis(sjoins,yzpz_file):
    '''
    亩产量疑点'
    '''
...
def writePoints2MultiKML(outfile0, df0, doc_name, fld_filed, lbr_field, coords_field, limit_num=None):
    '''
    写出kml
    outfile0:输出文件名
    df0: 写出对象
    doc_name: 文档名称
    fld_filed: 按哪个字段分文件夹
    lbr_field: 哪个字段作为标签/名称
    limit_num: 一个kml文件包含的df长度上限
    '''
    df0 = df0.reset_index()
    if limit_num is None:
        df = df0
        outfile = outfile0
        writePoints2kml(outfile, df, doc_name, fld_filed, lbr_field, coords_field)
    else:
        for i in range(0, len(df0), limit_num):
            df = df0.loc[i:min(i + limit_num - 1, len(df0)), :]
            outfile = outfile0.replace('.kml', f'-{int(i / limit_num)}.kml')
            writePoints2kml(outfile, df, doc_name, fld_filed, lbr_field, coords_field)
...
def create_description(row):
    ...
    for col in row.index:
        if col != 'geometry':
            desc_lines.append(f"{col}: {row[col]}")
    return "\n".join(desc_lines)
def writePoints2kml(outfile, df, doc_name, fld_filed, lbr_field, coords_field):
    '''
    写出kml
    outfile0:输出文件名
    df0: 写出对象
    ...
    flds = df[fld_filed].unique()
    for fld in flds:
        data = df[df[fld_filed] == fld]
        fold = KML.Folder(
            KML.name(fld)
        )
        for i, row in data.iterrows():
            plm = KML.Placemark(
                KML.name(row[lbr_field]),
                KML.description(row['description']),
                KML.Point(KML.coordinates(row[coords_field]))
            )
            fold.append(plm)
        doc.append(fold)
